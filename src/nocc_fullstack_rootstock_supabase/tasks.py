...
class SmartContractTasks:
    ...
    def setup_project(agent, project_name):
        return Task(
            description=f"Set up a new Rootstock smart contract project structure named '{project_name}'",
            agent=agent,
    ...
    def validate_contract(agent, contract):
        return Task(
            description=f"Validate the Rootstock smart contract: {contract}",
            agent=agent,
            expected_output="Validation result for the Rootstock smart contract."
        )

    @staticmethod
    def validate_tests(agent, tests):
        return Task(
            description=f"Validate the test suite for the Rootstock smart contract: {tests}",
            agent=agent,
    ...
    def finalize_project(agent, contract, tests):
        return Task(
            description=f"Validate and finalize the Rootstock smart contract project. Contract: {contract}, Tests: {tests}",
            agent=agent,
...
class FrontendTasks:
    ...
    @staticmethod
    def integrate_web3js(agent, frontend_code):
        return Task(
            description=f"Integrate Web3.js components into the frontend code for Rootstock interaction: {frontend_code}",
            agent=agent,
    ...
    def integrate_smart_contract(agent, frontend_code, contract):
        return Task(
            description=f"Integrate the Rootstock smart contract with the frontend. Frontend code: {frontend_code}, Contract: {contract}",
            agent=agent,
